...
void CStanding_StSleep::Update()
{
	...
	//[Jane Miller] I changed this because we thought the foot switches weren't reading correctly
	// and that we wanted the robot to go to strong standing when it was on the ground regardless
	// someone should probably change this to be a point where the robot accepts a button input to know
	// if it's on the ground or not. 

	// If Flame is placed on ground, go to the normal PD controller
	// logprintf("Foot switch states: left: %d right: %d\n",s.l().foot.back.state, s.r().foot.back.state);
